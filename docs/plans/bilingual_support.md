...
- **登録/編集フォーム (`word_form.html`)**:
  - ラベルを「単語 (Words): ※複数ある場合はカンマ(,)で区切ってください（最大5つまで）」に変更。
  - プレースホルダーに「例: Dependency Injection, 依存性の注入」のように表示し、複数入力を促す。
- **一覧画面 (`word_list.html`)**:
  - カラムを「単語 (Words)」「意味 (Meaning)」の構成に戻す。

## 3. 実装手順

1.  **Entityの修正**: `Word.java` を `term`/`definition` に更新。
2.  **Templateの修正**: UIラベルとバインディングの修正。

## 4. 検証項目

- [x] パターン1の入力（英語, 日本語 / 説明）が登録・表示できること。
- [x] パターン2の入力（英語 / 日本語訳）が登録・表示できること。
- [x] 一覧画面でカンマ区切りの単語が正しく表示されること。
